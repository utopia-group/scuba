...
def main():
    ...
    #projects = ['antlr','luindex','avrora','xalan','hsqldb','sunflow']
    projects = ['sunflow']
    ...
    for k in [2]:
        for proj in projects:
            ...
            runBenchmark(proj, benchLoc, str(k), str(k), str(k+1), 'cfa')
            runBenchmark(proj, benchLoc, str(k), str(k), str(k+1), 'obj')
...
def runBenchmark(benchName, benchLoc, kval, hval, scubaH, chordType):
    ...
    cmd = 'java -cp ' + chordLoc + ' -Dchord.work.dir=' + benchLoc + ' -Dchord.out.dir=' + chordDir + ' -Dchord.run.analyses=cipa-java,' + dlog + ',vv-refine-java,cspa-downcast-java,cspa-query-resolve-dlog,sum-java' + ' -Dchord.kcfa.k=' + kcfaK + ' -Dchord.kobj.k=' + kobjK + ' -Dchord.ctxt.kind=' + ctxtKind +  ' -Dchord.scuba.H=' + scubaH + ' -Dchord.scuba.proj=' + benchName  + ' chord.project.Boot'
    #cmd = 'java -cp ' + chordLoc + ' -Dchord.work.dir=' + benchLoc + ' -Dchord.out.dir=' + chordDir + ' -Dchord.run.analyses=cipa-java,' + dlog + ',vv-refine-java,cspa-downcast-java,cspa-query-resolve-dlog,sum-java' + ' -Dchord.kcfa.k=' + kcfaK + ' -Dchord.kobj.k=' + kobjK + ' -Dchord.scope.exclude=sun.,com.sun.,com.ibm.,org.apache.harmony.' ' -Dchord.ctxt.kind=' + ctxtKind +  ' -Dchord.scuba.H=' + scubaH + ' -Dchord.scuba.proj=' + benchName  + ' chord.project.Boot'

    #cmd = 'java -cp ' + chordLoc + ' -Dchord.work.dir=' + benchLoc + ' -Dchord.out.dir=' + chordDir + ' -Dchord.run.analyses=cipa-java,' + dlog + ',vv-refine-java,cspa-downcast-java,cspa-query-resolve-dlog,sum-java' + ' -Dchord.kcfa.k=' + kcfaK + ' -Dchord.kobj.k=' + kobjK + ' -Dchord.scope.exclude=java.awt.,java.util.,sun.,com.sun.,com.ibm.,org.apache.harmony.' ' -Dchord.ctxt.kind=' + ctxtKind +  ' -Dchord.scuba.H=' + scubaH + ' chord.project.Boot'
