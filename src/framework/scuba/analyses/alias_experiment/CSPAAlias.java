 ...
 * @author haiyan zhu
 */
@Chord(name = "cspa-alias-java", consumes = { "MV", "appsVV", "VValias",
		"cspaVValias", "cspaVVnotalias", "aliasVVpts", "notaliasVVpts",
		"VVRefined" })
public class CSPAAlias extends JavaAnalysis {
	...
	public void run() {
		...
		// only for print
		relaliasVVpts = (ProgramRel) ClassicProject.g().getTrgt("aliasVVpts");
	...
	private void VV() {
		...
		System.out
				.println("Total pairs that generate by derefrence of variables == "
						+ relappVV.size());
		...
		System.out.println("proved not alias/alias(cipa) :::: "
				...
				+ (double) relcspaVVnotalias.size() * 100
				/ (double) (relVV.size()) + "%");
	}
	...
	private void verbose_print2() {
		...
		Iterable<Pair<Register, Register>> vv = relcspaVValias
				.getAry2ValTuples();

		System.out.println("Total pairs that may alias  ::"
				+ relcspaVValias.size());
		int k = 0;
		for (Pair<Register, Register> ele : vv) {
			...
			for (Ctxt h1 : v1h) {
				...
				System.out.println("--Points to location :: "
						+ h1.head().toString() + "@ context " + h1.toString());
			}
			System.out.println(ele.val1.toString() + " : ");
			for (Ctxt h2 : v2h) {
				if (shareh.contains(h2))
					continue;
				System.out.println("--Points to location :: "
						+ h2.head().toString() + "@ context " + h2.toString());
			}
		}
	...
	private void verbose_print1() {
		...
		Iterable<Pair<Register, Register>> vv = relcspaVVnotalias
				.getAry2ValTuples();

		System.out
				.println("Total pairs that not alias based on not alias vv  ::"
						+ relcspaVVnotalias.size());
	...
	protected void VVRefined_print() {
