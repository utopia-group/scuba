...
@Chord(name = "cipa-downcast-java", consumes = { "cipaunsafeDowncast",
		"cipasafeDowncast", "cipadowncast",
...
public class CIPADowncastAnalysis extends JavaAnalysis {
	...
	public void run() {
		...
		// relMobjVarAsgnInst = (ProgramRel)
		// ClassicProject.g().getTrgt("MobjVarAsgnInst");

		relreachableM = (ProgramRel) ClassicProject.g().getTrgt("reachableM");

		relreachableappsM = (ProgramRel) ClassicProject.g().getTrgt(
				"reachableappsM");

		// relMcheckCastInst = (ProgramRel)
		// ClassicProject.g().getTrgt("McheckCastInst");
	...
	private void TestingAppMReachable() {
		...
		System.out
				.println("============ Starting test exclude libraries method ============= ");
		int i = 0;
		for (jq_Method m : ms) {
			System.out.println(i++ + " : ReachableAppm  == " + m.toString());
		}
		System.out
				.println("============ End testing exclude libraries method ============= ");
	}
	...
	private void print_result() {
		...
		System.out.println("Total Reachable Application Method :: "
				+ relreachableappsM.size());
		...
		System.out.println("Percentage unsafety ==> "
				+ (float) size_of_unsafety
				/ (float) (size_of_unsafety + size_of_safety));
