 ...
 * containing pairs (v,t) such that local variable v of reference type (say) t'
 * may be cast to reference type t which is not a supertype of t', and the cast,
 * as deemed by this analysis, is either provably safe or possibly unsafe,
 * respectively.
 * <p>
 * Recognized system properties:
 * <ul>
 * <li>All system properties recognized by abstract contexts analysis (see
 * {@link chord.analyses.alias.CtxtsAnalysis}).</li>
 * </ul>
 *
 *
 */
@Chord(name = "cspa-downcast-java", consumes = { "reachableCM", "reachableM",
		"downcast", "unsafeDowncast", "safeDowncast", "reachableappsCM" })
public class CSPADowncastAnalysis extends JavaAnalysis {
	...
	public void run() {
		...
		relreachableappsCM = (ProgramRel) ClassicProject.g().getTrgt(
				"reachableappsCM");

		// relMcheckCastInst =
		// (ProgramRel)ClassicProject.g().getTrgt("McheckCastInst");
		//
		// relMobjVarAsgnInst =
		// (ProgramRel)ClassicProject.g().getTrgt("MobjVarAsgnInst");
	...
	private void print_result() {
		...
		System.out.println("Total Reachable AppsM inside CM :: "
				+ relreachableappsCM.size());

		// System.out.println("Total checkcast instruction ::" +
		// relMcheckCastInst.size());
		//
		//
		//
		// System.out.println("Total objVarAssignment " +
		// relMobjVarAsgnInst.size());
		//
