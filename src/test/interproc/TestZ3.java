...
//			for(int i = 0; i < ctx.getNumTactics(); i++)
//				System.out.println("Tactic: " + ctx.getTacticNames()[i]);
			
			Solver mysolver = ctx.mkTactic("ctx-solver-simplify").getSolver();
	        ...
	        BoolExpr eq1 = ctx.mkEq(to, ctx.mkInt("3"));

	        BoolExpr eq2 = ctx.mkGe(to, ctx.mkInt("2"));
	        
	        BoolExpr eq4 = ctx.mkEq(ctx.mkInt("3"), ctx.mkInt("5"));
	        Tactic tactic = ctx.mkTactic("ctx-solver-simplify");
	        BoolExpr absorb1 = ctx.mkOr(new BoolExpr[] {eq, ctx.mkAnd(new BoolExpr[]{eq, eq2})});
	        BoolExpr absorb2 = ctx.mkAnd(new BoolExpr[] {eq, ctx.mkOr(new BoolExpr[]{eq, eq2})});
	        BoolExpr absorb3 = ctx.mkOr(new BoolExpr[] {eq, ctx.mkAnd(new BoolExpr[]{eq, eq1})});

	        BoolExpr exchange1 = ctx.mkAnd(new BoolExpr[] {ctx.mkOr(new BoolExpr[]{eq2, eq}), ctx.mkOr(new BoolExpr[]{eq, eq2})});
	        System.out.println("mycheck:" + mysolver.check(absorb1));
			...
			for (int j = 0; j < 1; j++) {
				...
				g4.reset();
				g4.add(absorb2);
				ar = applyTactic(ctx, tactic, g4);
				g4.reset();
				g4.add(exchange1);
				g4.reset();
				System.out.println("myhandle: " + absorb3);
				g4.add(absorb3);
				ar = applyTactic(ctx, tactic, g4);
			}
			long mend = System.nanoTime();
			StringUtil.reportSec("Ctx-simplify time-------: ", mstart, mend);
	
	        System.out.println("stupid1:" + eq);
	        System.out.println("stupid2:" + ctx.mkOr(new BoolExpr[] {eq, ctx.mkAnd(new BoolExpr[]{eq, eq2})}).simplify());
	        System.out.println("stupid2:" + ctx.mkOr(new BoolExpr[] {eq, ctx.mkAnd(new BoolExpr[]{eq, eq2})}));

	        System.out.println("stupid3:" + ctx.mkAnd(new BoolExpr[] {eq, ctx.mkOr(new BoolExpr[]{eq, eq2})}).simplify());
	        System.out.println("stupid3:" + ctx.mkAnd(new BoolExpr[] {eq, ctx.mkOr(new BoolExpr[]{eq, eq2})}));
	        
	        System.out.println("stupid4:" + ctx.mkAnd(new BoolExpr[] {ctx.mkOr(new BoolExpr[]{eq2, eq}), ctx.mkOr(new BoolExpr[]{eq, eq2})}));
	        System.out.println("stupid4:" + ctx.mkAnd(new BoolExpr[] {ctx.mkOr(new BoolExpr[]{eq2, eq}), ctx.mkOr(new BoolExpr[]{eq, eq2})}).simplify());

	        
	        BoolExpr sim = ctx.mkAnd(new BoolExpr[] { eq, eq2 });
	        
			System.out.println("Before simplify: " + sim);
			BoolExpr[] aa = new BoolExpr[2000000];
			solver.reset();
...
//			ApplyResult ar = ApplyTactic(ctx, ctx.mkTactic("smt"), g4);
			...
			StringUtil.reportSec("Tacitc time-------: " + aa.length + aa[1000000], tstart, tend);
	...
	ApplyResult applyTactic(Context ctx, Tactic t, Goal g) throws Z3Exception {
		...
		System.out.println("Application result: " + res.getSubgoals()[0].getFormulas()[0]);
